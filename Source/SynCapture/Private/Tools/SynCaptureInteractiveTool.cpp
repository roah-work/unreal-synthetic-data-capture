...
#include "SynCaptureInteractiveTool.h"
...
#define LOCTEXT_NAMESPACE "USynCaptureInteractiveTool"

/*
 * ToolBuilder
 */

UInteractiveTool* USynCaptureInteractiveToolBuilder::BuildTool(const FToolBuilderState & SceneState) const
{
	USynCaptureInteractiveTool* NewTool = NewObject<USynCaptureInteractiveTool>(SceneState.ToolManager);
...
void USynCaptureInteractiveTool::SetWorld(UWorld* World)
{
	check(World);
...
void USynCaptureInteractiveTool::Setup()
{
	...
	Properties = NewObject<USynCaptureInteractiveToolProperties>(this, "Measurement");
...
void USynCaptureInteractiveTool::OnUpdateModifierState(int ModifierID, bool bIsOn)
{
...
FInputRayHit USynCaptureInteractiveTool::CanBeginClickDragSequence(const FInputDeviceRay& PressPos)
{
...
void USynCaptureInteractiveTool::OnClickPress(const FInputDeviceRay& PressPos)
{
...
void USynCaptureInteractiveTool::OnClickDrag(const FInputDeviceRay& DragPos)
{
	UpdatePosition(DragPos.WorldRay);
}


FInputRayHit USynCaptureInteractiveTool::FindRayHit(const FRay& WorldRay, FVector& HitPos)
{
...
void USynCaptureInteractiveTool::UpdatePosition(const FRay& WorldRay)
{
...
void USynCaptureInteractiveTool::UpdateDistance()
{
	Properties->Distance = FVector::Distance(Properties->StartPoint, Properties->EndPoint);
}


void USynCaptureInteractiveTool::OnPropertyModified(UObject* PropertySet, FProperty* Property)
{
...
void USynCaptureInteractiveTool::Render(IToolsContextRenderAPI* RenderAPI)
{
